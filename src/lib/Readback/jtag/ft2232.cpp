...
ft2232::ft2232()
  ...
  , retries(0)
{
  description = string("Digilent Adept USB Device");
}
...
ft2232::open()
{
  ...
  char descstring[256] = "Digilent Adept USB Device";
  uint32_t vendor = FT2232_VENDOR_ID, product = FT2232_DEVICE_ID;
  uint32_t channel = 0;
...
ft2232::is_present()
{
  ...
  for(bus = usb_get_busses(); bus; bus = bus->next) {
    for(dev = bus->devices; dev; dev = dev->next) {
      if (dev->descriptor.idVendor == FT2232_VENDOR_ID && dev->descriptor.idProduct == FT2232_DEVICE_ID) {
	return true;
      }
    }
  }
...
void
ft2232::txrx_block(const uint8_t *tdi, uint8_t *tdo, int length, bool last)
{
...
void
ft2232::tx_tms(uint8_t *in, int length, int force)
{
...
void
ft2232::settype(int sub_type)
{
  subtype = sub_type;
}

uint32_t 
ft2232::readusb(uint8_t *rbuf, unsigned long len)
{
...
void
ft2232::mpsse_add_cmd(uint8_t const *buf, int len)
{
