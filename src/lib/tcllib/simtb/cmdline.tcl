...
proc ::cmdline::getoptions {arglistVar optlist {usage options:}} {
    ...
    if {[info exist result([set ?_sym])] || [info exists result(help)]} {
...
proc ::cmdline::usage {optlist {usage {options:}}} {
    ...
    foreach opt [concat $optlist \
		     {{- "Forcibly stop option processing"} {help "Print this message"}} [list [list [set ?_sym] "Print this message"]]] {
	set name [lindex $opt 0]
	if {[regsub -- {\.secret$} $name {} name] == 1} {
	    # Hidden option
	    continue
...
proc ::cmdline::typedGetoptions {arglistVar optlist {usage options:}} {
    ...
    if {[info exists result([set ?_sym])] || [info exists result(help)]} {
        error [typedUsage $optlist $usage]
    }
...
proc ::cmdline::typedUsage {optlist {usage {options:}}} {
    ...
    set str "[getArgv0] $usage\n"
    foreach opt [concat $optlist \
		     {{help "Print this message"}} [list [list [set ?_sym] "Print this message"]]] {
        set name [lindex $opt 0]
        if {[regsub -- {\.secret$} $name {} name] == 1} {
            # Hidden option
