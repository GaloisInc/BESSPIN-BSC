...
itcl::body type_browser::get_items {n} {
        ...
        if {$n == ""} {
                ...
                foreach i $tmp {
                        if {[catch "browsetype add \"$i\"" err]} {
                                puts stderr $err
                                continue 
                        }
                        lappend BSPEC(TYPES) $i
                }
                set rlist [browsetype list 0]
        } else {
                set rlist [browsetype list $n]
        }
        return $rlist
}
...
itcl::body type_browser::check_node {t} {
        ...
        if {[lsearch -regexp [browsetype detail $t] "position"] != -1} {
                change_menu_status view source normal
                change_item_menu_status "View_Source" normal
        }
        set p [lindex $BSPEC(TYPES) [expr [lindex [lindex [browsetype list 0] \
                [lsearch -regexp [browsetype list 0] $t]] 0] - 1]]
...
itcl::body type_browser::types_remove {} {
        ...
        if {[$BSPEC(TYPE_HIERARCHY) selection get] != ""} {
                ...
                set t [lindex $BSPEC(TYPES) [expr [lindex [lindex \
                        [browsetype list 0] [lsearch -regexp \
                        [browsetype list 0] $t]] 0] - 1]]
