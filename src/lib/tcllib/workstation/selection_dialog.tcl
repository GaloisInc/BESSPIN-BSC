...
proc set_select_type_bindings {} {
        bind .tl <Up> {
                ...
                if {[llength [$wi curselection]] == 1} {
                        ...
                        [lsearch [bpackage list] [$wi get_index $next]] != -1} {
...
proc set_select_type_settings {} {
        set tlist ""
        foreach i [browsepackage list 0] {
                foreach j [browsepackage list [lindex $i 0]] {
                        if {[bpackage types [lindex $j 1]] == ""} {
                                continue
                        } else {
                                lappend tlist [lindex $j 1]
                        }
                        foreach k [lsort [bpackage types [lindex $j 1]]] {
                                lappend tlist "    $k"
                        }
                }
        }
