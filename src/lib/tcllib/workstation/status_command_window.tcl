...
itcl::class status_command_window {
        ...
        ##
        # @brief setupt the interp with statically loaded modules and source
        # standard bluespec setup file
        #
        # @param s slave interpretator
        #
        proc setup_interp_with_extensions {s}

        ##
        # @brief Create and initialize the slave interpreter
        #
        # @param s slave interpretator
        #
        proc slave_init {s}
	...
	# insertion
        ...
        constructor {args} {
                ...
                set eval(slave) [status_command_window::slave_init shell]
                create_status_window
                create_command_line
                fraction 75 25
                # final setup of the command interp
                status_command_window::load_cmd_history
                setup_interp_with_extensions $eval(slave)
...
itcl::body status_command_window::get_commands {cmd} {
        ...
        global eval
        if {[llength $cmd] == 0} {
                set cmdlist [lsort [$eval(slave) eval info commands]]
                set nslist  [lsort [$eval(slave) eval namespace children]]
                ...
                if { $ns != "" && [$eval(slave) eval namespace exists $ns] } {
                        set t [namespace tail $cmd]
                        set cmdlist [lsort [$eval(slave) eval info commands ${ns}::${t}*]]
                        set nslist  [lsort [$eval(slave) eval namespace children $ns ${t}*]]
                } elseif { [$eval(slave) eval namespace exists $cmd] } {
                        set cmdlist [lsort [$eval(slave) eval info commands ${cmd}::*]]
                        set nslist  [lsort [$eval(slave) eval namespace children $cmd *]]
                } else {
                        set cmdlist [lsort [$eval(slave) eval info commands ${cmd}*]]
                        set nslist  [lsort [$eval(slave) eval namespace children :: ${cmd}*]]
                }
...
itcl::body status_command_window::evaluate {c} {
        ...
        if [catch {$eval(slave) eval $c} result] {
...
itcl::body status_command_window::slave_init {s} {
	interp create $s
...
itcl::body status_command_window::setup_interp_with_extensions  {s} {
        ...
        # Bring WS commands to the slave interp
        interp eval $s "source  \"$env(BLUESPECDIR)/tcllib/workstation/slave_namespace.tcl\""
        # source the user
...
itcl::body status_command_window::create_command_line {} {
        ...
        uplevel #0 "source \"$::env(BLUESPECDIR)/tcllib/workstation/bindings.tcl\""
}
...
itcl::body status_command_window::display_help {{c ""}} {
        ...
        if {$c == "-list"} {
                set cmdlist [lsort [$eval(slave) eval SlaveUtils::getNamespaceCommands WS]]
                foreach i $cmdlist {
...
proc status_command_window::open_fifo {} {
        ...
        } elseif { [info exists env(TEMP)] } { set tmp $env(TMP)
        } else { set tmp /tmp }
...
proc status_command_window::load_cmd_history {} {
        ...
        proc safe_load_history {} {
                ...
                global env
                global eval
                set hfile [open $env(HOME)/.bluespec/history.lst r]
...
proc status_command_window::save_cmd_history {} {
        ...
        if {! [catch "open $env(HOME)/.bluespec/history.lst w" hfile] } {
