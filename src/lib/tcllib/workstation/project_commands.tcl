...
namespace eval commands {
        ...
        namespace import -force ::Bluetcl::*
...
proc update_window_titles {} {
        global BSPEC
        wm title . "Bluespec Workstation$BSPEC(TITLE)"
...
proc set_project_options_in_interp {} {
        ...
        set scemipath %/Libraries/BlueNoC
        if  { [Bluetcl::flags show scemi-classic] == "-scemi-classic" } {
                set scemipath %/Libraries/Classic
        }
        commands::reset_flags
        # set defpath [split [lindex [Bluetcl::flags show p] 1] :]
        set defpath [list . %/Prelude %/Libraries]
        lappend defpath $scemipath
...
proc write_project_info {id} {
        puts $id "### Bluespec project file." 
        set d [exec date]
        puts $id "### Generated by Bluespec Workstation on $d\n"
...
# @param close whether to close the Viewer on Bluespec close or not, the
...
##
# @brief Specifies scemi options for the current project.
#
# @param enable specifies the activation of scemilink step.
# @param scemilink specifies options to scemilink.
#

proc set_scemi_options { enable scemilink } {
        global PROJECT
        set PROJECT(STATUS) ""
        set PROJECT(SCEMI_ENABLE) $enable
        set PROJECT(SCEMI_LINK_OPTIONS) $scemilink
}

##
# @brief Specifies scemi options for the current project.
#
# @param bsvtb specifies that the test module is bsv.
# @param file specifies the file name of the top bsv module.
# @param mod specifies the name of the top bsv module.
#

proc set_scemi_bsvtb { bsvtb file mod } {
        global PROJECT
        set PROJECT(STATUS) ""
        set PROJECT(SCEMI_BSVTB) $bsvtb
        set PROJECT(SCEMI_TB_FILE) $file
        set PROJECT(SCEMI_TB_MOD) $mod
}

##
# @brief Specifies scemi options for the current project.
#
# @param compbsc specifies that the bsv testbench will be compiled using bsc.
# @param opts specifies the compilation options.
#

proc set_scemi_bsvtb_comp_options { compbsc opt } {
        global PROJECT
        set PROJECT(STATUS) ""
        set PROJECT(SCEMI_COMPBSC) $compbsc
        set PROJECT(SCEMI_COMPOPT) $opt
}

##
# @brief Specifies scemi options for the current project.
#
# @param name specifies the name of the executable output.
# @param odir specifies the output directory.
# @param opts specifies the link options.
# @param runopts specifies the simulation run options.
#

proc set_scemi_bsvtb_link_options { name odir opts runopts } {
        global PROJECT
        set PROJECT(STATUS) ""
        set PROJECT(SCEMI_BSC_LINK_NAME) $name
        set PROJECT(SCEMI_BSC_LINK_ODIR) $odir
        set PROJECT(SCEMI_BSC_LINK_OPTS) $opts
        set PROJECT(SCEMI_BSC_LINK_RUNOPTS) $runopts
}

##
# @brief Specifies type of compilation for current project.
#
# @param type either "mkf" or "ccd" or "bsc"
#
proc set_tb_compilation_type {type} {
        global PROJECT
        set PROJECT(STATUS) ""
        set PROJECT(TB_COMP_TYPE) $type
}

##
# @brief Specifies make options for linking.
#
# @param makefile the compile/link makefile
# @param trg makefile target for compilation
# @param link_trg makefile target for linking
# @param sim_trg makefile target for simulation
#

proc set_scemi_make_options {makefile {trg ""} {link_trg ""} \
                                {sim_trg ""}} {
        global PROJECT
        set PROJECT(SCEMI_COMP_MAKEFILE) [get_real_path $makefile]
        set PROJECT(SCEMI_COMP_TARGET) $trg
        set PROJECT(SCEMI_BSC_LINKMK_TARGET) $link_trg
        set PROJECT(SCEMI_BSC_LINKMK_RUN) $sim_trg
}

##
# @brief Specifies scemi options linking tb for the current project.
#
# @param enable specifies the activation of scemilink step.
# @param scemilink specifies options to scemilink.
#

proc set_scemi_tb_link_options { cc link run } {
        global PROJECT
        set PROJECT(STATUS) ""
        set PROJECT(SCEMI_TB_CC_CC) $cc
        set PROJECT(SCEMI_TB_CC_LINK) $link
        set PROJECT(SCEMI_TB_CC_RUN) $run
}
...
proc show_help_content {} {
        global env
        open_browser $env(BLUESPECDIR)/tcllib/workstation/help/html/index.html
}
...
proc show_bsv_doc {} {
        global env
        open_browser $env(BLUESPECDIR)/../index.html
}
