...
itcl::class options_dialog {
        ...
        ##
        # @brief A list with names of "Build Testbench via Makefile",
	# "Link Testbench via custom command" checkbuttons
        #
        common tb_comp_type ""
        ...
        private {
                ...
                ##
                # @brief Sets scemi options for the current project 
                #
                method set_scemi_options {}
                ...
                # @brief Gets the current type of compilation  
                #
                method get_tb_comp_type {} {
                        global scemi_tb_comp_type_mkf
                        global scemi_tb_comp_type_ccd
                        global scemi_tb_comp_type_bsc
                        if {$scemi_tb_comp_type_mkf == 1} {
                                return mkf
                        } elseif {$scemi_tb_comp_type_ccd == 1} {
                                return ccd
                        } else {
				return bsc
			}
        ...
        ##
        # @brief process selection in scemi tab
        method do_scemi_enable { name } 
        method activate_scemi_fields {}
...
itcl::body options_dialog::add_tabnotebook {} {
        ...
        set scemi [$itk_component(tab) add -label "SCE-MI"]
...
itcl::body options_dialog::add_scemi_tab {} {
        global PROJECT
        set spacing 1
        add_togglebutton $scemi st1 scemi_enable "Enable SCE-MI Flow" [list $this do_scemi_enable scemi_enable] $spacing
        add_entryfield $scemi st1a scemi_options "Scemilink options for dut" $spacing

        add_togglebutton $scemi st2 scemi_bsvtb "BSV Testbench" [list $this do_scemi_enable scemi_bsvtb]  $spacing
        add_browse_entryfield $scemi st2a scemi_tb_file "Testbench file" {options_dialog::set_entry scemi_tb_file} $spacing
        add_entryfield $scemi st2b scemi_tb_mod "Testbench module" $spacing
        add_separator $scemi st2c

        add_togglebutton $scemi st3 scemi_compbsc "Compile Testbench via bsc" [list $this do_scemi_enable scemi_compbsc]  $spacing
        add_entryfield $scemi st3a scemi_compopt "Compile options" $spacing

        add_checkbutton $scemi st5 scemi_tb_comp_type_bsc "Link Testbench via bsc" [list $this do_scemi_enable scemi_tb_comp_type_bsc]  $spacing
        add_entryfield $scemi st5a scemi_bsc_link_name    "Output name" $spacing
        add_entryfield $scemi st5b scemi_bsc_link_odir    "Output directory" $spacing
        add_entryfield $scemi st5c scemi_bsc_link_opts    "Link options" $spacing
        add_entryfield $scemi st5d scemi_bsc_link_runopts "Run options" $spacing
        
        add_separator $scemi sts1
        add_checkbutton $scemi st4 scemi_tb_comp_type_mkf "Build Testbench via Makefile" [list $this do_scemi_enable scemi_tb_comp_type_mkf]  $spacing
        add_browse_entryfield $scemi st8 scemi_comp_makefile "Makefile" {options_dialog::set_entry scemi_comp_makefile} $spacing
        add_entryfield $scemi st4b scemi_comp_target "Compile target" $spacing
        add_entryfield $scemi st6b scemi_bsc_linkmk_target "Link target" $spacing
        add_entryfield $scemi st6d scemi_bsc_linkmk_run    "Simulate target" $spacing

        
        add_separator $scemi sts2
        add_checkbutton $scemi st7 scemi_tb_comp_type_ccd "Link Testbench via custom command" [list $this do_scemi_enable scemi_tb_comp_type_ccd]  $spacing
        add_entryfield $scemi st7a scemi_tb_cc_cc   "Compile command" $spacing
        add_entryfield $scemi st7b scemi_tb_cc_link "Link command" $spacing
        add_entryfield $scemi st7c scemi_tb_cc_run  "Simulate command" $spacing

        # set_entry needs these to be here.  Not sure why we cannot just grab $itk_component?
        lappend blist \
            [list  scemi_tb_file $itk_component(scemi_tb_file)] \
            [list  scemi_comp_makefile $itk_component(scemi_comp_makefile)]

        align_scemi_tab_labels
}

itcl::body options_dialog::align_scemi_tab_labels {} {
        iwidgets::Labeledwidget::alignlabels  \
            $itk_component(scemi_options) \
            $itk_component(scemi_tb_file) \
            $itk_component(scemi_tb_mod) \
            $itk_component(scemi_compopt) \
            $itk_component(scemi_comp_makefile) \
            $itk_component(scemi_comp_target) \
            $itk_component(scemi_bsc_link_name) \
            $itk_component(scemi_bsc_link_odir) \
            $itk_component(scemi_bsc_link_opts) \
            $itk_component(scemi_bsc_link_runopts) \
            $itk_component(scemi_bsc_linkmk_target) \
            $itk_component(scemi_bsc_linkmk_run) \
            $itk_component(scemi_tb_cc_cc) \
            $itk_component(scemi_tb_cc_link) \
            $itk_component(scemi_tb_cc_run) 
}
...
itcl::body options_dialog::add_waveform_viewer_tab {} {
        ...
        itk_component add wv_close {
                iwidgets::radiobox $wv.rb -orient horizontal -labeltext \
                        "Close viewer on Bluespec close" -labelpos nw
        } {
                keep -cursor -background
        }
...
# turn off or on lobal variables in the dialog which control the scemi options

itcl::body options_dialog::do_scemi_enable { name } {        
        # set or unset any toggles
        switch -exact $name {
                scemi_enable {}
                scemi_bsvtb {}
                scemi_compbsc {
                        if { $::scemi_compbsc } {
                                ...
                                set ::scemi_tb_comp_type_bsc off 
                        }
                }
                scemi_tb_comp_type_bsc {
                        set ::scemi_tb_comp_type_bsc on
                        set ::scemi_tb_comp_type_mkf off 
                        set ::scemi_tb_comp_type_ccd off 
                }
                scemi_tb_comp_type_mkf {
                        set ::scemi_tb_comp_type_bsc off
                        set ::scemi_tb_comp_type_mkf on
                        set ::scemi_tb_comp_type_ccd off 
                }
                scemi_tb_comp_type_ccd {
                        set ::scemi_tb_comp_type_bsc off 
                        set ::scemi_tb_comp_type_mkf off 
                        set ::scemi_tb_comp_type_ccd on

                }
        }
        activate_scemi_fields
}
itcl::body options_dialog::activate_scemi_fields {} {
        set on [list]
        set off [list]
        if { $::scemi_enable } {
                lappend on \
        ...
        foreach i $on {
                $itk_component($i)  configure -state normal
        }
        foreach i $off {
                $itk_component($i)  configure -state disabled
        }
...
itcl::body options_dialog::get_scemi_options {} {
        global PROJECT
        global scemi_enable
        global scemi_bsvtb
        global scemi_compbsc

        set scemi_enable $PROJECT(SCEMI_ENABLE)
        $itk_component(scemi_options) insert 0 $PROJECT(SCEMI_LINK_OPTIONS)
        # TODO $itk_component(soptions) insert 0 "$PROJECT(SCEMI_LINK_OPTIONS)"

        set scemi_bsvtb $PROJECT(SCEMI_BSVTB)
        $itk_component(scemi_tb_file) insert 0 $PROJECT(SCEMI_TB_FILE)
        $itk_component(scemi_tb_mod) insert 0 $PROJECT(SCEMI_TB_MOD)

        set scemi_compbsc $PROJECT(SCEMI_COMPBSC)
        $itk_component(scemi_compopt) insert 0 $PROJECT(SCEMI_COMPOPT)
 
        $itk_component(scemi_bsc_link_name) insert 0 $PROJECT(SCEMI_BSC_LINK_NAME)
        $itk_component(scemi_bsc_link_odir) insert 0 $PROJECT(SCEMI_BSC_LINK_ODIR)
        $itk_component(scemi_bsc_link_opts) insert 0 $PROJECT(SCEMI_BSC_LINK_OPTS)
        $itk_component(scemi_bsc_link_runopts) insert 0 $PROJECT(SCEMI_BSC_LINK_RUNOPTS)

        $itk_component(scemi_comp_makefile) insert 0 $PROJECT(SCEMI_COMP_MAKEFILE)

        $itk_component(scemi_comp_target) insert 0 $PROJECT(SCEMI_COMP_TARGET)
        $itk_component(scemi_bsc_linkmk_target) insert 0 $PROJECT(SCEMI_BSC_LINKMK_TARGET)
        $itk_component(scemi_bsc_linkmk_run) insert 0 $PROJECT(SCEMI_BSC_LINKMK_RUN)


        $itk_component(scemi_tb_cc_cc) insert 0 $PROJECT(SCEMI_TB_CC_CC)
        $itk_component(scemi_tb_cc_link) insert 0 $PROJECT(SCEMI_TB_CC_LINK)
        $itk_component(scemi_tb_cc_run) insert 0 $PROJECT(SCEMI_TB_CC_RUN)

        $itk_component(scemi_tb_comp_type_$PROJECT(TB_COMP_TYPE)) invoke
...
itcl::body options_dialog::set_scemi_options {} {
        commands::set_scemi_options $::scemi_enable \
                                    [get_entry scemi_options]

        commands::set_scemi_bsvtb $::scemi_bsvtb \
				  [get_entry scemi_tb_file] \
				  [get_entry scemi_tb_mod]
                                    

        commands::set_scemi_bsvtb_comp_options $::scemi_compbsc \
				  	       [get_entry scemi_compopt]

        commands::set_tb_compilation_type [$this get_tb_comp_type]

        commands::set_scemi_bsvtb_link_options [get_entry scemi_bsc_link_name] \
				               [get_entry scemi_bsc_link_odir] \
				               [get_entry scemi_bsc_link_opts] \
				               [get_entry scemi_bsc_link_runopts]
                                    
        commands::set_scemi_make_options  [get_entry scemi_comp_makefile] \
	                               	  [get_entry scemi_comp_target] \
	                               	  [get_entry scemi_bsc_linkmk_target] \
	                               	  [get_entry scemi_bsc_linkmk_run]

        commands::set_scemi_tb_link_options  [get_entry scemi_tb_cc_cc] \
                                   	     [get_entry scemi_tb_cc_link] \
                                   	     [get_entry scemi_tb_cc_run]
}
