...
proc add_bsc_tb_options {} {
        ...
        if {$PROJECT(SCEMI_COMPOPT) != ""} {
                append f "$PROJECT(SCEMI_COMPOPT) "
                Bluetcl::flags set $PROJECT(SCEMI_COMPOPT)
        }
        append f "-p [get_project_paths] "
        return $f
}
...
proc compile_get_cmd {} {
        ...
        # Also do the testbench compile
        if { $PROJECT(SCEMI_ENABLE) } {
          if {$PROJECT(SCEMI_COMPBSC)} {
		append cmd ";\n" [commands::get_bsc_tb_compile_command]
	    } else { 
	    switch $PROJECT(TB_COMP_TYPE) {
		mkf {
                        append cmd ";\n" [commands::get_compile_tb_via_make_command]
		}
		ccd {
			append cmd ";\n" [commands::get_compile_tb_via_custom_command]
		}
	    }
	  }
	}
...
# @brief Compiles the project via make
#
proc get_compile_tb_via_make_command {} {
        global PROJECT
        if {$PROJECT(SCEMI_COMP_MAKEFILE) == ""} {
                error_message "Makefile for testbench is not specified."
                return
        }
        set mt [customize_custom_command $PROJECT(SCEMI_COMP_TARGET)]

        return "make -f $PROJECT(SCEMI_COMP_MAKEFILE) $mt"
}
...
proc get_bsc_compile_command {} {
        ...
        if { $PROJECT(SCEMI_ENABLE) } {
                if { $PROJECT(TOP_MODULE) == "" } {
                        error_message "Top module must be set for Sce-Mi link"
                        return ""
                }
                append cmd ";\n"
                append cmd "scemilink"
                append cmd " -p [get_project_paths]"
                append cmd " -bdir $PROJECT(COMP_BDIR):+"
                append cmd " --simdir $PROJECT(COMP_SIMDIR)"
                append cmd " $PROJECT(SCEMI_LINK_OPTIONS)"
                append cmd " $PROJECT(TOP_MODULE)"
	}
        return $cmd
}
...
# @brief Returns the command for compile via bsc
#
proc get_bsc_tb_compile_command {} {
        global PROJECT
        if {$PROJECT(SCEMI_TB_FILE) == ""} {
                error_message "The top Testbench file is not specified in the SceMi flow"
                return ""
        }
        set f ""
        append f [add_bsc_tb_options]
        if {$PROJECT(SCEMI_TB_MOD) != ""} {
                append f "-g $PROJECT(SCEMI_TB_MOD) "
        }
        set cmd "bsc -u $f $PROJECT(SCEMI_TB_FILE) "
        return $cmd
}
...
#
proc execute_tb_bsc {} {
        if {[set cmd [get_bsc_tb_compile_command]] != ""} {
                commands::action_start Compiling
                execute_in_shell $cmd "commands::action_finished Compile"
        }
}
...
##
# @brief Sets bluesim options for link using bsv as the scemi testbench
#
# @return string with options
#
proc set_link_tb_bluesim_options {} {
        global PROJECT
        set exe "bsc -e $PROJECT(SCEMI_TB_MOD) -sim -scemiTB "
        append exe "-o $PROJECT(LINK_OUTDIR)/$PROJECT(SCEMI_BSC_LINK_NAME) "
        if { $PROJECT(COMP_SIMDIR) != "" } {
                append exe "-simdir $PROJECT(COMP_SIMDIR) "
        }
        append exe "-p [get_project_paths] "
        append exe "-bdir $PROJECT(COMP_BDIR) " 
        append exe "$PROJECT(SCEMI_BSC_LINK_OPTS) "
        # append exe "$PROJECT(COMP_BDIR)/$PROJECT(SCEMI_TB_MOD).ba"
        return $exe
}
...
# @return string with verilog options
#
proc set_link_tb_verilog_options {} {
        global PROJECT
        set out "$PROJECT(LINK_OUTDIR)/$PROJECT(SCEMI_BSC_LINK_NAME)"
        set vdir $PROJECT(COMP_VDIR)
        set topv $vdir/$PROJECT(SCEMI_TB_MOD).v
        set exe "bsc -e $PROJECT(SCEMI_TB_MOD) -verilog -o $out -vdir $vdir \
                -vsim $PROJECT(SIM_NAME) \
                $PROJECT(SCEMI_BSC_LINK_OPTS) $topv"
        return $exe
}
...
proc link_tb_via_bsc {} {
        ...
        switch $PROJECT(COMP_BSC_TYPE) {
                bluesim {
                        set exe [set_link_tb_bluesim_options]
                }
                verilog {
                        set exe [set_link_tb_verilog_options]
                }
                default { set exe "" }
        }
        customize_custom_command $exe
...
# @return the link command
#
proc link_tb_via_make {} {
        global PROJECT
        set lt [customize_custom_command $PROJECT(SCEMI_BSC_LINKMK_TARGET)]
        return "make -f $PROJECT(SCEMI_COMP_MAKEFILE) $lt"
}
...
proc customize_custom_command {exe} {
        ...
        if { ![catch "exec $env(BLUESPECDIR)/bin/bsenv c++_family" res] } {
                set cppfamily $res
                set exe [regsub -all "%F" $exe $cppfamily]
        } else {
                puts stderr "Cannot determine c++ compiler.\n$res"
        }
        ...
        # SCE-MI flow
        set exe [regsub -all "%SCM" $exe $PROJECT(SCEMI_TB_MOD)]
        set exe [regsub -all "%SCP" $exe $PROJECT(SCEMI_TB_FILE)] 
...
# @return the link command
#
proc link_tb_via_custom_command {} {
        global PROJECT
        if {$PROJECT(SCEMI_TB_CC_LINK) == ""} {
                #error_message "There is no link command given for SceMi testbench"
                return
        } 
        return [customize_custom_command "$PROJECT(SCEMI_TB_CC_LINK)"]
}
...
# @return the link command
#
proc get_compile_tb_via_custom_command {} {
        global PROJECT
        if {$PROJECT(SCEMI_TB_CC_CC) == ""} {
                #error_message "There is no command given for compiling SceMi testbench"
                return
        }
        return [customize_custom_command "$PROJECT(SCEMI_TB_CC_CC)"]
}
...
proc link_get_cmd {} {
        ...
        # Also do the testbench linking
	if {$PROJECT(SCEMI_ENABLE)} {
                switch $PROJECT(TB_COMP_TYPE) {
                        bsc {
                                append cmd ";\n" [commands::link_tb_via_bsc]
                        }
                        mkf {
                                append cmd ";\n" [commands::link_tb_via_make]
                        }
                        ccd {
                                append cmd ";\n" [commands::link_tb_via_custom_command]
                        }
                }
	}
        return $cmd
...
# @return the simulation command
#
proc simulate_tb_via_bsc {} {
        global PROJECT
        set out $PROJECT(LINK_OUTDIR)/$PROJECT(SCEMI_BSC_LINK_NAME)
        if {$PROJECT(COMP_BSC_TYPE) == "bluesim"} {
                return  "$out $PROJECT(SCEMI_BSC_LINK_RUNOPTS)"
        } elseif {$PROJECT(COMP_BSC_TYPE) == "verilog"} {
                return  "$out $PROJECT(SCEMI_BSC_LINK_RUNOPTS)" 
       }
}
...
# @return the simulation command
#
proc simulate_tb_via_make {} {
        global PROJECT
#        set lmo [customize_custom_command $PROJECT(LINK_MAKE_OPTIONS)]
        set lst [customize_custom_command $PROJECT(SCEMI_BSC_LINKMK_RUN)]
        return "make -f $PROJECT(SCEMI_COMP_MAKEFILE) $lst"
}
...
# @return the simulation command
#
proc simulate_via_tb_custom_command {} {
        global PROJECT
        return [customize_custom_command "$PROJECT(SCEMI_TB_CC_RUN)"]
}
...
proc simulate_get_cmd_scemi {} {
        global PROJECT
        set cmd ""
	if {$PROJECT(SCEMI_ENABLE)} {
                # Add a little sleep to make sure the DUT is instantiated and fully awake
                set cmd "sleep 5;\n"
                switch $PROJECT(TB_COMP_TYPE) {
                        bsc {
                                append cmd [commands::simulate_tb_via_bsc]
                        }
                        mkf {
                                append cmd [commands::simulate_tb_via_make]
                        }
                        ccd {
                                append cmd [commands::simulate_via_tb_custom_command]
                        }
                }
        }
        return $cmd
...
proc simulate {} {
	...
	execute_in_shell $cmd "commands::action_finished Simulation"
        ...
        set scmd [simulate_get_cmd_scemi]
        if { $scmd != "" } {
                # Force update here to allow dut command to start and print.
                update
                commands::action_start "Simulating SCE-MI Testbench"
                execute_in_shell $scmd ""
        }
}
...
proc comp_link_and_sim {} {
        ...
        set scmd2 [simulate_get_cmd_scemi]
        if { $scmd2 == "" } {
                append cmd " ;\n" $scmd
        } else {
                append cmd " ;\n( " $scmd " &) ;\n" $scmd2
