...
source "$script_dir/ddr2_v11_0_p0_report_timing_core.tcl"
...
foreach inst $instances {
	...
	if {[llength $instances] <= 1} {
		set fbasename "${::GLOBAL_ddr2_v11_0_p0_corename}"
	} else {
		set fbasename "${::GLOBAL_ddr2_v11_0_p0_corename}_${inst_id}"
	}
	...
	foreach opc $opcs {
		...
		if {!$in_gui && [array exists assumptions_cache] &&  [info exists assumptions_cache(${::GLOBAL_ddr2_v11_0_p0_corename}-$inst)] } {
			set assumptions_valid $assumptions_cache(${::GLOBAL_ddr2_v11_0_p0_corename}-$inst)
			...
			set assumptions_cache(${::GLOBAL_ddr2_v11_0_p0_corename}-$inst) $assumptions_valid
		}	
		...
		perform_resync_timing_analysis $opcs $opcname $inst ${::GLOBAL_ddr2_v11_0_p0_corename} $family $DQS_min_max $io_std $interface_type $period pins t summary MP IP board fpga SSN
	...
	set summary [lsort -command ddr2_v11_0_p0_sort_proc $summary]

	set f -1
	if { [get_operating_conditions_number] == 0 } {
		set f [open $fname w]

		puts $f "Core: ${::GLOBAL_ddr2_v11_0_p0_corename} - Instance: $inst"
		puts $f "Path, Setup Margin, Hold Margin"
	} else {
		set f [open $fname a]
	}

	post_message -type info "Core: ${::GLOBAL_ddr2_v11_0_p0_corename} - Instance: $inst"
