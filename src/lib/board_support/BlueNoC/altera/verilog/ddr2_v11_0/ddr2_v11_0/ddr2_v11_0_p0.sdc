...
#      in the ddr2_v11_0_p0_timing.tcl script.
#    * The helper routines are defined in ddr2_v11_0_p0_pin_map.tcl
#
# NOTE
# ----
...
source "$script_dir/ddr2_v11_0_p0_timing.tcl"
...
if { $::GLOBAL_ddr2_v11_0_p0_phy_use_micro_timing } {
	##################
	#                #
	# QUERIED TIMING #
	#                #
	...
	set io_standard "$::GLOBAL_ddr2_v11_0_p0_io_standard CLASS I"

	# This is the peak-to-peak jitter on the whole read capture path
	set DQSpathjitter [expr [get_io_standard_node_delay -dst DQDQS_JITTER -io_standard $io_standard -parameters [list IO $::GLOBAL_ddr2_v11_0_p0_io_interface_type] -in_fitter]/1000.0]
	set DQSpathjitter_setup_prop [expr [get_io_standard_node_delay -dst DQDQS_JITTER_DIVISION -io_standard $io_standard -parameters [list IO $::GLOBAL_ddr2_v11_0_p0_io_interface_type] -in_fitter]/100.0]

	set tJITper [expr [get_io_standard_node_delay -dst MEM_CK_PERIOD_JITTER -io_standard $io_standard  -parameters [list IO $::GLOBAL_ddr2_v11_0_p0_io_interface_type] -in_fitter -period $t(CK)]/2000.0]
	...
	# Minimum delay on data output pins
	set data_output_min_delay [ round_3dp [ expr - $t(DH) - $fpga(tPLL_JITTER) -$fpga(tPLL_PSERR) - $board(intra_DQS_group_skew) - $SSN(rel_pullin_o) ]]

	# Maximum delay on data output pins
	set data_output_max_delay [ round_3dp [ expr $t(DS) + $fpga(tPLL_JITTER) + $fpga(tPLL_PSERR) + $board(intra_DQS_group_skew) + $SSN(rel_pushout_o) ]]

	# Maximum delay on data input pins
	set data_input_max_delay [ round_3dp [ expr $t(DQSQ) + $board(intra_DQS_group_skew) + $SSN(rel_pushout_i) ]]

	# Minimum delay on data input pins
	set data_input_min_delay [ round_3dp [ expr -$t(DCD) - $board(intra_DQS_group_skew) - $SSN(rel_pullin_i) ]]

	# Minimum delay on address and command paths
	set ac_min_delay [ round_3dp [ expr - $t(IH) -$fpga(tPLL_JITTER) - $fpga(tPLL_PSERR) - $board(intra_addr_ctrl_skew) + $board(addresscmd_CK_skew) ]]

	# Maximum delay on address and command paths
	set ac_max_delay [ round_3dp [ expr $t(IS) +$fpga(tPLL_JITTER) + $fpga(tPLL_PSERR) + $board(intra_addr_ctrl_skew) + $board(addresscmd_CK_skew)]]
}
...
if { ! [ info exists ddr2_v11_0_p0_sdc_cache ] } {
	set ddr2_v11_0_p0_sdc_cache 1
	ddr2_v11_0_p0_initialize_ddr_db ddr2_v11_0_p0_ddr_db
} else {
	if { $debug } {
...
# following loop
set instances [ array names ddr2_v11_0_p0_ddr_db ]
foreach { inst } $instances {
	if { [ info exists pins ] } {
		# Clean-up stale content
		unset pins
	}
	array set pins $ddr2_v11_0_p0_ddr_db($inst)
	...
	set fpga(tDQS_PHASE_JITTER) [ expr [ get_integer_node_delay -integer $::GLOBAL_ddr2_v11_0_p0_dqs_delay_chain_length -parameters {IO MAX HIGH} -src DQS_PHASE_JITTER -in_fitter ] / 1000.0 ]

	# Phase Error on DQS paths. This parameter is queried at run time
	set fpga(tDQS_PSERR) [ expr [ get_integer_node_delay -integer $::GLOBAL_ddr2_v11_0_p0_dqs_delay_chain_length -parameters {IO MAX HIGH} -src DQS_PSERR -in_fitter ] / 1000.0 ]
	...
	if { $debug } {
		...
		post_message -type info "SDC: -----------------"
		post_message -type info "SDC:    DQS Phase: $::GLOBAL_ddr2_v11_0_p0_dqs_delay_chain_length"
	...
	# PLL Clock Names
	set local_pll_afi_clk "${inst}|ddr2_v11_0_p0_pll_afi_clk"
	set local_pll_mem_clk "${inst}|ddr2_v11_0_p0_pll_mem_clk"
	set local_pll_write_clk "${inst}|ddr2_v11_0_p0_pll_write_clk"
	set local_pll_addr_cmd_clk "${inst}|ddr2_v11_0_p0_pll_addr_cmd_clk"
	set local_pll_afi_half_clk "${inst}|ddr2_v11_0_p0_pll_afi_half_clk"
	set local_pll_avl_clock "${inst}|ddr2_v11_0_p0_pll_avl_clock"
	set local_pll_config_clock "${inst}|ddr2_v11_0_p0_pll_config_clock"
	...
	# W A R N I N G !
	# The PLL parameters are statically defined in the ddr2_v11_0_p0_parameters.tcl
	...
	# apply those changes to the PLL parameters in the ddr2_v11_0_p0_parameters.tcl

	set pll_i 5
	create_generated_clock -add -name "$local_pll_afi_clk" -source $pll_ref_clock -multiply_by $::GLOBAL_ddr2_v11_0_p0_pll_mult(0) -divide_by $::GLOBAL_ddr2_v11_0_p0_pll_div(0) -phase $::GLOBAL_ddr2_v11_0_p0_pll_phase(0) $pll_afi_clock
	if {[string compare -nocase $pll_afi_clock $pll_ck_clock]==0} {
		set local_pll_mem_clk $local_pll_afi_clk
	} else {
		create_generated_clock -add -name "$local_pll_mem_clk" -source $pll_ref_clock -multiply_by $::GLOBAL_ddr2_v11_0_p0_pll_mult(1) -divide_by $::GLOBAL_ddr2_v11_0_p0_pll_div(1) -phase $::GLOBAL_ddr2_v11_0_p0_pll_phase(1) $pll_ck_clock
	}
	create_generated_clock -add -name "$local_pll_write_clk" -source $pll_ref_clock -multiply_by $::GLOBAL_ddr2_v11_0_p0_pll_mult(2) -divide_by $::GLOBAL_ddr2_v11_0_p0_pll_div(2) -phase $::GLOBAL_ddr2_v11_0_p0_pll_phase(2) $pll_write_clock
	create_generated_clock -add -name "$local_pll_addr_cmd_clk" -source $pll_ref_clock -multiply_by $::GLOBAL_ddr2_v11_0_p0_pll_mult(3) -divide_by $::GLOBAL_ddr2_v11_0_p0_pll_div(3) -phase $::GLOBAL_ddr2_v11_0_p0_pll_phase(3) $pll_ac_clock
	create_generated_clock -add -name "$local_pll_afi_half_clk" -source $pll_ref_clock -multiply_by $::GLOBAL_ddr2_v11_0_p0_pll_mult(4) -divide_by $::GLOBAL_ddr2_v11_0_p0_pll_div(4) -phase $::GLOBAL_ddr2_v11_0_p0_pll_phase(4) $pll_afi_half_clock
	set avl_pll_clk_i $pll_i
	if {[string compare -nocase $pll_afi_clock $pll_avl_clock]==0} {
		set local_pll_avl_clock $local_pll_afi_clk
	} else {	
		create_generated_clock -add -name "$local_pll_avl_clock" -source $pll_ref_clock -multiply_by $::GLOBAL_ddr2_v11_0_p0_pll_mult($pll_i) -divide_by $::GLOBAL_ddr2_v11_0_p0_pll_div($pll_i) -phase $::GLOBAL_ddr2_v11_0_p0_pll_phase($pll_i) $pll_avl_clock
	}
	incr pll_i
	create_generated_clock -add -name "$local_pll_config_clock" -source $pll_ref_clock -multiply_by $::GLOBAL_ddr2_v11_0_p0_pll_mult($pll_i) -divide_by $::GLOBAL_ddr2_v11_0_p0_pll_div($pll_i) -phase $::GLOBAL_ddr2_v11_0_p0_pll_phase($pll_i) $pll_config_clock
	incr pll_i
	...
	create_generated_clock -add -name "${inst}|memphy_leveling_clk" -master_clock [get_clocks $local_pll_write_clk] -source $pll_write_clock -phase $::GLOBAL_ddr2_v11_0_p0_leveling_capture_phase [ get_pins $leveling_pins ]
	...
	set_multicycle_path -from [get_registers ${prefix}*usequencer|*sequencer_phy_mgr_inst|phy_mux_sel] -to [remove_from_collection [get_keepers *] [get_registers ${prefix}*usequencer|*sequencer_phy_mgr_inst|phy_mux_sel]] -setup 3
	set_multicycle_path -from [get_registers ${prefix}*usequencer|*sequencer_phy_mgr_inst|phy_mux_sel] -to [remove_from_collection [get_keepers *] [get_registers ${prefix}*usequencer|*sequencer_phy_mgr_inst|phy_mux_sel]] -hold 2
	set_multicycle_path -through [get_nets ${inst}*usequencer|*rw_mgr_core_inst|group_select[*]] -setup 3
	set_multicycle_path -through [get_nets ${inst}*usequencer|*rw_mgr_core_inst|group_select[*]] -hold 2
    ...
    set_min_delay -from [get_registers ${prefix}*read_data_out*] -to $fifo_wrdata_reg [ round_3dp [expr -$t(CK) + 0.20 ]]
...
if { $::GLOBAL_ddr2_v11_0_p0_phy_use_micro_timing } {
	if {(($::quartus(nameofexecutable) ne "quartus_fit") && ($::quartus(nameofexecutable) ne "quartus_map"))} {
		set dqs_clocks [ddr2_v11_0_p0_get_all_instances_dqs_pins ddr2_v11_0_p0_ddr_db]
...
add_ddr_report_command "source [list [file join [file dirname [info script]] ${::GLOBAL_ddr2_v11_0_p0_corename}_report_timing.tcl]]"
