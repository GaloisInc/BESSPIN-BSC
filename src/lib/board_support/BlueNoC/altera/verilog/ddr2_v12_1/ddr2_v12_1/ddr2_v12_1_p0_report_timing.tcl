...
source "$script_dir/ddr2_v12_1_p0_report_timing_core.tcl"
...
foreach inst $instances {
	...
	set clk_to_write_d [ddr2_v12_1_p0_traverse_to_ddio_out_pll_clock [lindex $all_dq_pins 0] msg_list]
	...
	# DQS_phase offset
	set dqs_phase [ ddr2_v12_1_p0_get_dqs_phase $dqs_pins ]

	# Get the interface type (HPAD or VPAD)
	set interface_type [ddr2_v12_1_p0_get_io_interface_type $all_dq_pins]
	...
	set io_std [ddr2_v12_1_p0_get_io_standard [lindex $dqs_pins 0]]
	...
	if {[llength $instances] <= 1} {
		set fbasename "${::GLOBAL_ddr2_v12_1_p0_corename}"
	} else {
		set fbasename "${::GLOBAL_ddr2_v12_1_p0_corename}_${inst_id}"
	}
	...
	foreach opc $opcs {
		...
		set model_corner [ddr2_v12_1_p0_get_model_corner]
		...
		if {!$in_gui && [array exists assumptions_cache] &&  [info exists assumptions_cache(${::GLOBAL_ddr2_v12_1_p0_corename}-$inst)] } {
			set assumptions_valid $assumptions_cache(${::GLOBAL_ddr2_v12_1_p0_corename}-$inst)
			...
			if { $::GLOBAL_ddr2_v12_1_p0_phy_use_micro_timing } {
				set assumptions_valid [ddr2_v12_1_p0_verify_flexible_timing_assumptions $inst pins $mem_if_memtype]
			} else {
				set assumptions_valid [ddr2_v12_1_p0_verify_high_performance_timing_assumptions $inst pins $mem_if_memtype]
			}
			set assumptions_cache(${::GLOBAL_ddr2_v12_1_p0_corename}-$inst) $assumptions_valid
		}	
		...
		if { $::GLOBAL_ddr2_v12_1_p0_phy_use_micro_timing } {
			ddr2_v12_1_p0_perform_flexible_write_launch_timing_analysis $opcs $opcname $inst $family $DQS_min_max $interface_type $max_package_skew $dll_length $period pins t summary MP IP board
		} else {
			ddr2_v12_1_p0_perform_macro_write_launch_timing_analysis $opcs $opcname $inst $dll_length $interface_type pins t summary IP ISI board
		}
		...
		if { $::GLOBAL_ddr2_v12_1_p0_phy_use_micro_timing } {
			ddr2_v12_1_p0_perform_flexible_read_capture_timing_analysis $opcs $opcname $inst $family $DQS_min_max $io_std $interface_type $max_package_skew $dqs_phase $period $all_dq_pins pins t summary MP IP board fpga
		} else {
			ddr2_v12_1_p0_perform_macro_read_capture_timing_analysis $opcs $opcname $inst $dqs_phase $dll_length $interface_type $all_dq_pins pins t summary IP board
		}
		ddr2_v12_1_p0_perform_resync_timing_analysis $opcs $opcname $inst ${::GLOBAL_ddr2_v12_1_p0_corename} $family $DQS_min_max $io_std $interface_type $period pins t summary MP IP board fpga SSN

		#######################################
		# PHY and Address/command Analyses

		ddr2_v12_1_p0_perform_ac_analyses  $opcs $opcname $inst $DQS_min_max pins t summary IP		
		ddr2_v12_1_p0_perform_phy_analyses $opcs $opcname $inst $inst_controller pins t summary IP
		
		#######################################
		# Write Levelling Analysis (UniPHY levelling option picked)
		if { $::GLOBAL_ddr2_v12_1_p0_phy_use_micro_timing } {
			ddr2_v12_1_p0_perform_flexible_write_levelling_timing_analysis $opcs $opcname $inst $family $period $dll_length $interface_type $tJITper $tJITdty $tDCD $pll_steps pins t summary MP IP SSN board ISI
		} else {
			ddr2_v12_1_p0_perform_macro_write_levelling_timing_analysis $opcs $opcname $inst $family $period $dll_length $interface_type $tJITper $tJITdty $tDCD $pll_steps pins t summary MP IP SSN board ISI
		}

		#######################################
		# Bus Turnaround Time Analysis
		if { $::GLOBAL_ddr2_v12_1_p0_phy_use_micro_timing } {
			ddr2_v12_1_p0_perform_flexible_bus_turnaround_time_analysis $opcs $opcname $inst $family $period $dll_length $interface_type $tJITper $tJITdty $tDCD $pll_steps pins t summary MP IP SSN board ISI
		} 
	...
	set summary [lsort -command ddr2_v12_1_p0_sort_proc $summary]

	set f -1
	if { [ddr2_v12_1_p0_get_operating_conditions_number] == 0 } {
		set f [open $fname w]

		puts $f "Core: ${::GLOBAL_ddr2_v12_1_p0_corename} - Instance: $inst"
	...
	post_message -type info "Core: ${::GLOBAL_ddr2_v12_1_p0_corename} - Instance: $inst"
	...
	foreach summary_line $summary {
		...
		if {$su != "--"} {
			set su [ ddr2_v12_1_p0_round_3dp $su]
		}
		if {$hold != "--"} {
			set hold [ ddr2_v12_1_p0_round_3dp $hold]
		}
