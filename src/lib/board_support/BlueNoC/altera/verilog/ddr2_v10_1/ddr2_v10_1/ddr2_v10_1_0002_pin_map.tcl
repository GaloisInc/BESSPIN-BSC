...
# ddr2_v10_1_0002_pin_assignments.tcl and ddr2_v10_1_0002.sdc scripts. 
#
		...
		if {[array size results_array] == 1} {
			...
			if {[array size results_array] == 1} {
				...
				if {[array size pll_clk_results_array] == 1} {
					...
					traverse_fanin_up_to_depth $pll_inclk_id is_node_type_pin clock results_array 4
					if {[array size results_array] == 1} {
						set pin_id [lindex [array names results_array] 0]
						set result $pin_id
					} else {
...
proc ddr2_v10_1_0002_sort_proc {a b} {
	set idxs [list 1 2 0]
	foreach i $idxs {
		set ai [lindex $a $i]
		set bi [lindex $b $i]
...
proc get_dqs_phase { dqs_pins } {
	...
	if { $dqs_phase_setting != $::GLOBAL_ddr2_v10_1_0002_dqs_delay_chain_length } {
		...
		post_message -type critical_warning "   Parameter value: $::GLOBAL_ddr2_v10_1_0002_dqs_delay_chain_length"
...
proc ddr2_v10_1_0002_get_ddr_pins { instname allpins } {
	upvar allpins pins
	...
	for { set i 0 } { $i < $::GLOBAL_ddr2_v10_1_0002_number_of_dqs_groups } { incr i } {
		...
		lappend leveling_pins "${instname}|controller_phy_inst|memphy_top_inst|umemphy|uio_pads|dq_ddio[$i].ubidir_dq_dqs|${dqs_inst}enable_ctrl|clk"
		...
		set q_group [ list ]
		for { set j 0 } { $j < $::GLOBAL_ddr2_v10_1_0002_dqs_group_size } { incr j } { 
			set index [ expr $i * $::GLOBAL_ddr2_v10_1_0002_dqs_group_size + $j ]
		...
		if { [llength $q_group] != $::GLOBAL_ddr2_v10_1_0002_dqs_group_size} { 
			post_sdc_message critical_warning "Could not find correct number of D pins for K pin $i. \
				Found [llength $q_group] pins. Expecting ${::GLOBAL_ddr2_v10_1_0002_dqs_group_size}." 
		}
	...
	set pins(avl_ck_pins) ${instname}|controller_phy_inst|memphy_top_inst|umemphy|usequencer|sequencer_rw_mgr_inst|state.STATE_RW_IDLE
	set pins(config_ck_pins) ${instname}|controller_phy_inst|memphy_top_inst|umemphy|usequencer|sequencer_scc_mgr_inst|scc_state_curr.STATE_SCC_IDLE
	...
	if {$pll_ck_clock_id == -1} {
		foreach {msg_type msg} $msg_list {
			post_message -type $msg_type "ddr2_v10_1_0002_pin_map.tcl: $msg"
		}
		post_message -type critical_warning "ddr2_v10_1_0002_pin_map.tcl: Failed to find PLL clock for pins [join $pins(ck_pins)]"
	} else {
	...
	if {$pll_write_clock_id == -1} {
		...
		post_message -type critical_warning "ddr2_v10_1_0002_pin_map.tcl: Failed to find PLL clock for pins [ join [ join $pins(q_groups) ]]"
	} else {
	...
	if {$pll_ac_clock_id == -1} {
		...
		post_message -type critical_warning "ddr2_v10_1_0002_pin_map.tcl: Failed to find PLL clock for pins [join $pins(add_pins)]"
	} else {
	...
	if {$pll_afi_half_clock_id == -1} {
		...
		post_message -type critical_warning "ddr2_v10_1_0002_pin_map.tcl: Failed to find PLL clock for pins [join $pins(afi_half_ck_pins)]"
	} else {
	...
	if {$pll_avl_clock_id == -1} {
		...
		post_message -type critical_warning "ddr2_v10_1_0002_pin_map.tcl: Failed to find PLL clock for pins [join $pins(avl_ck_pins)]"
	} else {
	...
	if {$pll_config_clock_id == -1} {
		...
		post_message -type critical_warning "ddr2_v10_1_0002_pin_map.tcl: Failed to find PLL clock for pins [join $pins(config_ck_pins)]"
	} else {
	...
	if {$pll_ref_clock_id == -1} {
		post_message -type critical_warning "ddr2_v10_1_0002_pin_map.tcl: Failed to find PLL reference clock"
	} else {
	...
	set read_capture_ddio [list "$prefix|*:controller_phy_inst|*:memphy_top_inst|*:umemphy|*:uio_pads|*:dq_ddio\[*\].ubidir_dq_dqs|*input_path_gen\[*\].capture_reg*" "$prefix|*:controller_phy_inst|*:memphy_top_inst|*:umemphy|*:uio_pads|*:dq_ddio\[*\].ubidir_dq_dqs|*read_data_out\[*\]"]
	if { ! $entity_names_on } {
		set read_capture_ddio [list "$instname|controller_phy_inst|memphy_top_inst|umemphy|uio_pads|dq_ddio\[*\].ubidir_dq_dqs|*input_path_gen\[*\].capture_reg*" "$instname|controller_phy_inst|memphy_top_inst|umemphy|uio_pads|dq_ddio\[*\].ubidir_dq_dqs|*read_data_out\[*\]"]
	}
	...
	set seq_reset_reg $prefix|*:controller_phy_inst|*:memphy_top_inst|*:umemphy|*:usequencer|*
	if { ! $entity_names_on } {
		set seq_reset_reg $instname|controller_phy_inst|memphy_top_inst|umemphy|usequencer|*
	}
	...
	set fifo_wrdata_reg $prefix|*:controller_phy_inst|*:memphy_top_inst|*:umemphy|*:uread_datapath|*:read_buffering[*].uread_fifo*|data_stored[*][*]
	if { ! $entity_names_on } {
		set fifo_wrdata_reg $instname|controller_phy_inst|memphy_top_inst|umemphy|uread_datapath|read_buffering[*].uread_fifo*|data_stored[*][*]
	}
...
proc ddr2_v10_1_0002_initialize_ddr_db { ddr_db_par } {
	upvar $ddr_db_par local_ddr_db

	global ::GLOBAL_ddr2_v10_1_0002_corename

	post_sdc_message info "Initializing DDR database for CORE $::GLOBAL_ddr2_v10_1_0002_corename"
	set instance_list [get_core_instance_list $::GLOBAL_ddr2_v10_1_0002_corename]

	foreach instname $instance_list {
		post_sdc_message info "Finding port-to-pin mapping for CORE: $::GLOBAL_ddr2_v10_1_0002_corename INSTANCE: $instname"

		ddr2_v10_1_0002_get_ddr_pins $instname allpins

		ddr2_v10_1_0002_verify_ddr_pins allpins

		set local_ddr_db($instname) [ array get allpins ]
	}
}

proc ddr2_v10_1_0002_verify_ddr_pins { pins_par } {
	upvar $pins_par pins
	...
	if { $::GLOBAL_ddr2_v10_1_0002_number_of_dm_pins != $counted_dm_pins } {
		post_message -type critical_warning "Unexpected number of detected DM pins: $counted_dm_pins"
		post_message -type critical_warning "   expected: $::GLOBAL_ddr2_v10_1_0002_number_of_dm_pins"
	}
...
proc ddr2_v10_1_0002_get_all_instances_div_names { ddr_db_par } {
	upvar $ddr_db_par local_ddr_db
...
proc ddr2_v10_1_0002_get_all_instances_dqs_pins { ddr_db_par } {
	upvar $ddr_db_par local_ddr_db
...
proc ddr2_v10_1_0002_dump_all_pins { ddr_db_par } {
	upvar $ddr_db_par local_ddr_db

	set instnames [ array names local_ddr_db ]

	set filename "${::GLOBAL_ddr2_v10_1_0002_corename}_all_pins.txt"
	...
	puts $FH "# PIN MAP for core < $::GLOBAL_ddr2_v10_1_0002_corename >"
	puts $FH "#"
	puts $FH "# Generated by ${::GLOBAL_ddr2_v10_1_0002_corename}_pin_assignments.tcl"
	puts $FH "#"
...
proc ddr2_v10_1_0002_dump_static_pin_map { ddr_db_par filename } {
	upvar $ddr_db_par local_ddr_db
	...
	puts $FH "# AUTO-GENERATED static pin map for core < $::GLOBAL_ddr2_v10_1_0002_corename >"
	puts $FH ""
	puts $FH "proc ${::GLOBAL_ddr2_v10_1_0002_corename}_initialize_static_ddr_db { ddr_db_par } {"
