			...
			for {set k 0} {$k != [llength $clk_pin_prefix_list]} {incr k} {
				if {$j == $k} { 																						;#constraints applied only if indices are matching
					...
					if {$phy_lane1_size > 0} {
						...
						for {set i 0} {$i != $stop} {incr i} {
									set_false_path -from "$get_clk_prefix|altpcie_a10_hip_pipen1b:altpcie_a10_hip_pipen1b|skp_det_g3:gen_det_skp_os.skp_det\[$i].inst|skp_pat_det_g3_ps" -to "$get_clk_prefix|altpcie_a10_hip_pipen1b:altpcie_a10_hip_pipen1b|skp_pat_det_g3_ps_r1\[$i]"
						}
						# SKP Specific - clock domain transfer for the skp pattern detected pulse			
						set_false_path -from "$get_clk_prefix|altpcie_a10_hip_pipen1b:altpcie_a10_hip_pipen1b|altpcie_sc_bitsync_node:reset_status_altpcie_sc_bitsync|altpcie_sc_bitsync:altpcie_sc_bitsync|g_bitsync.g_bitsync2.sync_regsb\[0]" -to "$get_clk_prefix|altpcie_a10_hip_pipen1b|reset_status_1sync\[*]"
