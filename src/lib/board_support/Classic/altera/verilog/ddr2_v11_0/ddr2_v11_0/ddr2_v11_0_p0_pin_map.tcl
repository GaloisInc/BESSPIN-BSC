...
# ddr2_v11_0_p0_pin_assignments.tcl and ddr2_v11_0_p0.sdc scripts. 
#
...
source [file join $script_dir ddr2_v11_0_p0_parameters.tcl]
...
proc ddr2_v11_0_p0_sort_proc {a b} {
	set idxs [list 1 2 0]
	foreach i $idxs {
		set ai [lindex $a $i]
		set bi [lindex $b $i]
...
proc get_dqs_phase { dqs_pins } {
	...
	if { $dqs_phase_setting != $::GLOBAL_ddr2_v11_0_p0_dqs_delay_chain_length } {
		...
		post_message -type critical_warning "   Parameter value: $::GLOBAL_ddr2_v11_0_p0_dqs_delay_chain_length"
...
proc ddr2_v11_0_p0_get_ddr_pins { instname allpins } {
	...
	for { set i 0 } { $i < $::GLOBAL_ddr2_v11_0_p0_number_of_dqs_groups } { incr i } {
		...
		set q_group [ list ]
		for { set j 0 } { $j < $::GLOBAL_ddr2_v11_0_p0_dqs_group_size } { incr j } { 
			set index [ expr $i * $::GLOBAL_ddr2_v11_0_p0_dqs_group_size + $j ]
		...
		if { [llength $q_group] != $::GLOBAL_ddr2_v11_0_p0_dqs_group_size} { 
			post_sdc_message critical_warning "Could not find correct number of D pins for K pin $i. \
				Found [llength $q_group] pins. Expecting ${::GLOBAL_ddr2_v11_0_p0_dqs_group_size}." 
		}
	...
	if {$pll_ck_clock_id == -1} {
		foreach {msg_type msg} $msg_list {
			post_message -type $msg_type "ddr2_v11_0_p0_pin_map.tcl: $msg"
		}
		post_message -type critical_warning "ddr2_v11_0_p0_pin_map.tcl: Failed to find PLL clock for pins [join $pins(ck_pins)]"
	} else {
	...
	if {$pll_write_clock_id == -1} {
		...
		post_message -type critical_warning "ddr2_v11_0_p0_pin_map.tcl: Failed to find PLL clock for pins [ join [ join $pins(q_groups) ]]"
	} else {
	...
	if {$pll_ac_clock_id == -1} {
		...
		post_message -type critical_warning "ddr2_v11_0_p0_pin_map.tcl: Failed to find PLL clock for pins [join $pins(add_pins)]"
	} else {
	...
	if {$pll_afi_half_clock_id == -1} {
		...
		post_message -type critical_warning "ddr2_v11_0_p0_pin_map.tcl: Failed to find PLL clock for pins [join $pins(afi_half_ck_pins)]"
	} else {
	...
	if {$pll_avl_clock_id == -1} {
		...
		post_message -type critical_warning "ddr2_v11_0_p0_pin_map.tcl: Failed to find PLL clock for pins [join $pins(avl_ck_pins)]"
	} else {
	...
	if {$pll_config_clock_id == -1} {
		...
		post_message -type critical_warning "ddr2_v11_0_p0_pin_map.tcl: Failed to find PLL clock for pins [join $pins(config_ck_pins)]"
	} else {
	...
	if {$pll_ref_clock_id == -1} {
		post_message -type critical_warning "ddr2_v11_0_p0_pin_map.tcl: Failed to find PLL reference clock"
	} else {
...
proc ddr2_v11_0_p0_initialize_ddr_db { ddr_db_par } {
	upvar $ddr_db_par local_ddr_db

	global ::GLOBAL_ddr2_v11_0_p0_corename

	post_sdc_message info "Initializing DDR database for CORE $::GLOBAL_ddr2_v11_0_p0_corename"
	set instance_list [get_core_instance_list $::GLOBAL_ddr2_v11_0_p0_corename]
	# set local_ddr_db(instance_list) $instance_list

	foreach instname $instance_list {
		post_sdc_message info "Finding port-to-pin mapping for CORE: $::GLOBAL_ddr2_v11_0_p0_corename INSTANCE: $instname"

		ddr2_v11_0_p0_get_ddr_pins $instname allpins

		ddr2_v11_0_p0_verify_ddr_pins allpins

		set local_ddr_db($instname) [ array get allpins ]
	}
}

proc ddr2_v11_0_p0_verify_ddr_pins { pins_par } {
	upvar $pins_par pins
	...
	if { $::GLOBAL_ddr2_v11_0_p0_number_of_dm_pins != $counted_dm_pins } {
		post_message -type critical_warning "Unexpected number of detected DM pins: $counted_dm_pins"
		post_message -type critical_warning "   expected: $::GLOBAL_ddr2_v11_0_p0_number_of_dm_pins"
	}
...
proc ddr2_v11_0_p0_get_all_instances_div_names { ddr_db_par } {
	upvar $ddr_db_par local_ddr_db
...
proc ddr2_v11_0_p0_get_all_instances_dqs_pins { ddr_db_par } {
	upvar $ddr_db_par local_ddr_db
...
proc ddr2_v11_0_p0_dump_all_pins { ddr_db_par } {
	upvar $ddr_db_par local_ddr_db

	set instnames [ array names local_ddr_db ]

	set filename "${::GLOBAL_ddr2_v11_0_p0_corename}_all_pins.txt"
	...
	puts $FH "# PIN MAP for core < $::GLOBAL_ddr2_v11_0_p0_corename >"
	puts $FH "#"
	puts $FH "# Generated by ${::GLOBAL_ddr2_v11_0_p0_corename}_pin_assignments.tcl"
	puts $FH "#"
...
proc ddr2_v11_0_p0_dump_static_pin_map { ddr_db_par filename } {
	upvar $ddr_db_par local_ddr_db
	...
	puts $FH "# AUTO-GENERATED static pin map for core < $::GLOBAL_ddr2_v11_0_p0_corename >"
	puts $FH ""
	puts $FH "proc ${::GLOBAL_ddr2_v11_0_p0_corename}_initialize_static_ddr_db { ddr_db_par } {"
