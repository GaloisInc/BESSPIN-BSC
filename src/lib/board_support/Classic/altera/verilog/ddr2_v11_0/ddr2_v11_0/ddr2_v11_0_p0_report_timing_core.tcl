...
proc perform_flexible_write_launch_timing_analysis {opcs opcname inst family DQS_min_max interface_type max_package_skew dll_length period pin_array_name timing_parameters_array_name summary_name MP_name IP_name board_name} {
	...
	if {($result == 1)} {
		...
		foreach dqpins $pins(q_groups) {
			...
			set dqs_periphery_node ${inst}|controller_phy_inst|memphy_top_inst|umemphy|uio_pads|dq_ddio[${group_number}].ubidir_dq_dqs|altdq_dqs2_inst|*dqs_alignment|clk
			...
			set output_buffer_node ${inst}|controller_phy_inst|memphy_top_inst|umemphy|uio_pads|dq_ddio[${group_number}].ubidir_dq_dqs|altdq_dqs2_inst|*obuf*_0|i
			...
			foreach dqpin $dqdmpins {
				...
				if {$isdmpin == 1} {
					if {$family == "arria ii"}  {
						...
						set dq_periphery_node ${inst}|controller_phy_inst|memphy_top_inst|umemphy|uio_pads|dq_ddio[${group_number}].ubidir_dq_dqs|altdq_dqs2_inst|*extra_output_pad_gen[0].data_alignment|clk
					}
				} else {
					if {$family == "arria ii"}  {
						set dq_periphery_node ${inst}|controller_phy_inst|memphy_top_inst|umemphy|uio_pads|uwrite_pads|*|dq_ddio[*].ubidir_dq_dqs|altdq_dqs2_inst|bidir_dq_*_output_ddio_out_inst|muxsel
					} else {
						set dq_periphery_node ${inst}|controller_phy_inst|memphy_top_inst|umemphy|uio_pads|dq_ddio[${group_number}].ubidir_dq_dqs|altdq_dqs2_inst|*output_path_gen[${dq_index}].data_alignment|clk
					}
				}
				...
				# Find the DQ clock node before the periphery
				if {$isdmpin == 1} {
					set output_buffer_node_dq ${inst}|controller_phy_inst|memphy_top_inst|umemphy|uio_pads|dq_ddio[${group_number}].ubidir_dq_dqs|altdq_dqs2_inst|*extra_output_pad_gen[0].obuf_1|i
				} else {
					set output_buffer_node_dq ${inst}|controller_phy_inst|memphy_top_inst|umemphy|uio_pads|dq_ddio[${group_number}].ubidir_dq_dqs|altdq_dqs2_inst|pad_gen[${dq_index}].data_out|i
				}
...
proc perform_flexible_read_capture_timing_analysis {opcs opcname inst family DQS_min_max io_std interface_type max_package_skew dqs_phase period all_dq_pins pin_array_name timing_parameters_array_name summary_name MP_name IP_name board_name fpga_name} {
	...
	if {($result == 1)} {
		...
		foreach qpins $pins(q_groups) {
			...
			set input_buffer_node "${inst}|controller_phy_inst|memphy_top_inst|umemphy|uio_pads|dq_ddio[${group_number}].ubidir_dq_dqs|altdq_dqs2_inst|*strobe_in|o" 
			...
			foreach qpin $qpins {
				...
				set input_buffer_node_dq ${inst}|controller_phy_inst|memphy_top_inst|umemphy|uio_pads|dq_ddio[${group_number}].ubidir_dq_dqs|altdq_dqs2_inst|*pad_gen[${q_index}].data_in|o
