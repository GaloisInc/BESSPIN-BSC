...
source "$script_dir/ddr2_v10_1_0002_report_timing_core.tcl"
...
if { ![string match *GUI*DDR* [get_current_timequest_report_folder]] && !$signoff_mode} {
	read_sdc
...
foreach inst $instances {
	...
	if {[llength $instances] <= 1} {
		set fbasename "${::GLOBAL_ddr2_v10_1_0002_corename}"
	} else {
		set fbasename "${::GLOBAL_ddr2_v10_1_0002_corename}_${inst_id}"
	}
	...
	foreach opc $opcs {
		...
		if {!$in_gui && [array exists assumptions_cache] &&  [info exists assumptions_cache(${::GLOBAL_ddr2_v10_1_0002_corename}-$inst)] } {
			set assumptions_valid $assumptions_cache(${::GLOBAL_ddr2_v10_1_0002_corename}-$inst)
			...
			if { $::GLOBAL_ddr2_v10_1_0002_phy_use_micro_timing } {
				...
				set assumptions_valid [verify_high_performance_timing_assumptions $inst pins $mem_if_memtype]
			}
			set assumptions_cache(${::GLOBAL_ddr2_v10_1_0002_corename}-$inst) $assumptions_valid
		}	
		...
		if { $::GLOBAL_ddr2_v10_1_0002_phy_use_micro_timing } {
			...
			perform_macro_write_launch_timing_analysis $opcs $opcname $inst $dll_length $interface_type pins t summary IP ISI board
		}
		...
		if { $::GLOBAL_ddr2_v10_1_0002_phy_use_micro_timing } {
			...
			perform_macro_read_capture_timing_analysis $opcs $opcname $inst $dqs_phase $dll_length $interface_type $all_dq_pins pins t summary IP board
		}
		perform_resync_timing_analysis $opcs $opcname $inst ${::GLOBAL_ddr2_v10_1_0002_corename} $family $DQS_min_max $io_std $interface_type $period pins t summary MP IP board fpga SSN

		########################################
		## PHY and Address/command Analyses

		perform_ac_analyses  $opcs $opcname $inst pins t summary IP		
		perform_phy_analyses $opcs $opcname $inst pins t summary IP
		...
		if { $::GLOBAL_ddr2_v10_1_0002_phy_use_micro_timing } {
			...
			perform_macro_write_levelling_timing_analysis $opcs $opcname $inst $family $period $dll_length $interface_type $tJITper $tJITdty $tDCD $pll_steps pins t summary MP IP SSN board ISI
		}
	}
	...
	set summary [lsort -command ddr2_v10_1_0002_sort_proc $summary]

	set f -1
	if { [get_operating_conditions_number] == 0 } {
		set f [open $fname w]

		puts $f "Core: ${::GLOBAL_ddr2_v10_1_0002_corename} - Instance: $inst"
		puts $f "Path, Setup Margin, Hold Margin"
	} else {
		set f [open $fname a]
	}

	post_message -type info "Core: ${::GLOBAL_ddr2_v10_1_0002_corename} - Instance: $inst"
