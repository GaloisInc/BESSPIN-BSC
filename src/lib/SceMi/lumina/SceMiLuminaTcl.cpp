...
public:
  ...
  void init(const char *instname, const char *paramfile)
  {
    // Allocate
    try {
      m_pControl    = new SceMiLuminaControl(instname, paramfile);
...
extern "C" {
  extern int Semulite_scemi_Init(Tcl_Interp *interp);
  extern int Semulite_scemi_Unload(Tcl_Interp *interp, int flags);
...
int Semulite_scemi_Unload (Tcl_Interp *interp, int flags)
{
...
// function name must match package library name
int Semulite_scemi_Init(Tcl_Interp *interp)
{
...
{
  string instname, paramfile, signalname, res;
  ...
  static const cmd_struct cmds_str[] = {
    {  "init",            SEMULITE_INIT,               "<instname> <paramfile>" }
  ...
  switch(command) {
    case SEMULITE_INIT:
      ...
      paramfile = Tcl_GetString(objv[3]);

      // Initialize readback
      try {
	pGlobal->init(instname.c_str(), paramfile.c_str());
      } catch (const exception &err) {
