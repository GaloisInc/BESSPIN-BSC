...
SceMi::SceMi(const SceMiParameters* params, SceMiEC* ec)
{
  ...
  // instantiate all pipe endpoints listed in the params file
  unsigned int index = parameters->NumberOfObjects("InputPipe", ec);
  while (index--)
  {
    const char* name = parameters->AttributeStringValue("InputPipe",index,"PipeName",ec);
    const char* xactor = parameters->AttributeStringValue("InputPipe",index,"TransactorName",ec);
    unsigned int width = parameters->AttributeIntegerValue("InputPipe",index,"PipeWidth",ec);
    unsigned int depth = parameters->AttributeIntegerValue("InputPipe",index,"PipeDepth",ec);
    const char* visibility = parameters->AttributeStringValue("InputPipe",index,"Visibility",ec);
    unsigned int pipe_num = parameters->AttributeIntegerValue("InputPipe",index,"PipeNum",ec);

    Visibility vis = (!strcmp(visibility,"Fifo"))      ? FIFO
                   : (!strcmp(visibility,"Immediate")) ? IMMEDIATE
                   : (!strcmp(visibility,"Deferred"))  ? DEFERRED
                   :                                     INVALID;

    if ((name == NULL) || (xactor == NULL) || (visibility == NULL) || (vis == INVALID)) {
      if (name == NULL)
        raiseError ("SceMi::SceMi", "InputPipe is missing PipeName attribute", SceMiError, ec);
      if (xactor == NULL)
        raiseError ("SceMi::SceMi", "InputPipe is missing TransactorName attribute", SceMiError, ec);
      if (visibility == NULL)
        raiseError ("SceMi::SceMi", "InputPipe is missing Visibility attribute", SceMiError, ec);
      if (vis == INVALID)
        raiseError ("SceMi::SceMi", "InputPipe has unknown Visibility attribute", SceMiError, ec);
      for (std::vector<SceMiPipe*>::iterator piter = pipes.begin(); piter != pipes.end(); ++piter) {
        SceMiPipe* pipe = *piter;
        delete pipe;
      }
    ...
    SceMiPipe* pipe = new SceMiInputPipe(xactor,name,pipe_num,width,depth,vis,link);
    pipes.push_back(pipe);
  }

  index = parameters->NumberOfObjects("OutputPipe", ec);
  while (index--)
  {
    const char* name = parameters->AttributeStringValue("OutputPipe",index,"PipeName",ec);
    const char* xactor = parameters->AttributeStringValue("OutputPipe",index,"TransactorName",ec);
    unsigned int width = parameters->AttributeIntegerValue("OutputPipe",index,"PipeWidth",ec);
    unsigned int depth = parameters->AttributeIntegerValue("OutputPipe",index,"PipeDepth",ec);
    const char* visibility = parameters->AttributeStringValue("OutputPipe",index,"Visibility",ec);
    unsigned int pipe_num = parameters->AttributeIntegerValue("OutputPipe",index,"PipeNum",ec);
    ...
    if ((name == NULL) || (xactor == NULL) || (visibility == NULL) || (vis == INVALID)) {
      if (name == NULL)
        raiseError ("SceMi::SceMi", "OutputPipe is missing PipeName attribute", SceMiError, ec);
      if (xactor == NULL)
        raiseError ("SceMi::SceMi", "OutputPipe is missing TransactorName attribute", SceMiError, ec);
      if (visibility == NULL)
        raiseError ("SceMi::SceMi", "OutputPipe is missing Visibility attribute", SceMiError, ec);
      if (vis == INVALID)
        raiseError ("SceMi::SceMi", "OutputPipe has unknown Visibility attribute", SceMiError, ec);
      for (std::vector<SceMiPipe*>::iterator piter = pipes.begin(); piter != pipes.end(); ++piter) {
        SceMiPipe* pipe = *piter;
        delete pipe;
      }
    ...
    SceMiPipe* pipe = new SceMiOutputPipe(xactor,name,pipe_num,width,depth,vis,link);
    pipes.push_back(pipe);
  }

  // initialize vectors for in and out proxies
  inProxies.resize( parameters->NumberOfObjects("MessageInPort") );
  outProxies.resize( parameters->NumberOfObjects("MessageOutPort") );
}
...
void SceMi::SetPointer(SceMi* ptr, SceMiEC* ec)
{
  the_scemi_object = ptr;
}
                       ...
                       SceMiEC* ec)
{
  ...
  for (std::vector<SceMiPipe*>::iterator iter = pipes.begin(); iter != pipes.end(); ++ iter ) {
  ...
  while (!exit_service_loop)
  {
    ...
    else if (!pending_from_hw.empty())
    {
      ...
      if ((pkt->channel < outProxies.size()) && proxy && proxy->Binding().Receive)
      {
...
SceMiPipe* SceMi::get_scemi_pipe(const char* endpoint_path)
{
  if (the_scemi_object) {
    std::vector<SceMiPipe*>::iterator it;
...
SceMiPipe* SceMi::get_scemi_pipe(unsigned int pipe_num)
{
  if (the_scemi_object) {
