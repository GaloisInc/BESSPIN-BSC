...
#include "SceMiPipe.h"       /* Transaction pipes implementation */
...
typedef enum { NONE, SIMULATOR, TB } SceMiServiceOwner;
...
class SceMi {
 ...
 private:
  ...
  SceMiServiceOwner service_owner;
  ...
  // record of transaction pipes
  std::vector<SceMiPipe*> pipes;
 ...
 public:
    ...
    static void SetPointer(SceMi* ptr, SceMiEC* ec = NULL);
    ...
    SceMiServiceOwner GetServiceOwner() { return service_owner; }
    void              SetServiceOwner(SceMiServiceOwner owner) { service_owner = owner; }
    ...
    // Special extension to return platform specific object
    static void *scemi_extension_access_vendor_platform();

    // Access to the individual pipe interfaces
    SceMiPipe* get_scemi_pipe(const char* endpoint_path);
    SceMiPipe* get_scemi_pipe(unsigned int pipe_num);
};
