...
class WaitQueueT {
 ...
 private:
  ...
  WaitQueueT & operator= (const   WaitQueueT &);
 ...
 public:
  ...
  /// \return  returns true if the Request has been received,  false if
  /// a timeout occurred.
  bool waitForDataTimed (struct timespec *expiration) const {
    bool res = false;
    int status;
  ...
  /// \return  returns true if the Request has been received,  false if
  /// a timeout occurred.
  bool waitForDataTimed (const time_t & delta_seconds,  const long & delta_microseconds=0) const {
    struct timespec ts;
    setTimeout(ts, delta_seconds, delta_microseconds);
  ...
  /// \return  returns true if the Request has been received,  false if
  /// a timeout occurred.
  bool peekT (T &t, const time_t & delta_seconds,  const long & delta_microseconds=0) const {
    struct timespec ts;
    setTimeout(ts, delta_seconds, delta_microseconds);
    return peekT (t, &ts);
  }
