...
data Decoded = DHelp Flags       -- Display the public help message
             ...
             | DError [EMsg]     -- Report a list of errors
               -- No source is given, but print-flag, verbose, or expiry
...
exitWithHelp :: ErrorHandle -> String -> [String] -> String -> String -> IO ()
exitWithHelp errh prog args cd expiryMsg =
    hPutStrLn stdout (helpMessage Visible prog args cd) >>
    hPutStrLn stdout (expiryMsg) >>
    exitOK errh

exitWithHelpHidden :: ErrorHandle -> String -> [String] -> String -> String -> IO ()
exitWithHelpHidden errh prog args cd expiryMsg =
    hPutStrLn stdout (helpMessage Hidden prog args cd) >>
    hPutStrLn stdout (expiryMsg) >>
    exitOK errh

-- This is the help message without the expiry
...
adjustFinalFlags warns0 errs0 flags0 =
	...
	-- using the appropriate SceMi directory.

	scemi_dir = if (genSceMiClassic flags0)
                     then bspecdir ++ "/Libraries/Classic"
		     else bspecdir ++ "/Libraries/BlueNoC"
        -- XXX make sure this is in sync with the default value
...
externalFlags = [
        ...
        ("licenseWarning",
         (Arg "days"
             (\f s -> case (mread s) of
                          Nothing -> Right (cmdPosition, EIntegerArgFlag "-licenseWarning")
                          Just arg -> Left (f {licenseWarning = arg}))
             (Just (FRTString (show . licenseWarning))),
          "sets the number of days before a license expires to issue a warning", Visible)),

        ("license-type",
         (Arg "type"
             (\f s -> case s of
                        "Any" ->  Left (f {licenseType = Any })
                        "any" ->  Left (f {licenseType = Any })
                        "Seat" ->  Left (f {licenseType = Seat })
                        "seat" ->  Left (f {licenseType = Seat })
                        "Float" ->  Left (f {licenseType = Floating })
                        "float" ->  Left (f {licenseType = Floating })
                        "Floating" ->  Left (f {licenseType = Floating })
                        "floating" ->  Left (f {licenseType = Floating })
                        "BSimOnly" ->  Left (f {licenseType = BlueSimOnly })
                        "BlueSimOnly" ->  Left (f {licenseType = BlueSimOnly })
                        _          ->  Right (cmdPosition, EBadArgFlag "-license-type" s ["Any", "Seat", "Floating", "BlueSimOnly"])
             )
             (Just (FRTString (show . licenseType))),
          "sets the type of license (Seat or Floating or BlueSimOnly or Any) for bsc", Visible)),
        ...
        ("print-expiration",
         (Toggle (\f x -> f {printExpiry=x}) (showIfTrue printExpiry),
          "print the expiration date and exit", Visible)),
        ...
        ("scemi",
         (Toggle (\f x -> f {genSceMi=x}) (showAlways genSceMi),
          "build a model using Sce-Mi link", Visible)),

        ("scemiTB",
         (Toggle (\f x -> f {genSceMiTB=x}) (showAlways genSceMiTB),
          "build a testbench model using Sce-Mi link", Visible)),

        ("scemi-classic",
         (Toggle (\f x -> f {genSceMiClassic=x}) (showAlways genSceMiClassic),
          "build a model or a testbench model using the Classic Sce-Mi infrastructure", Visible)),
        ...
        ("show-license-detail",
         (Toggle (\f x -> f {showLicense=x}) (showIfTrue showLicense),
          "show more details regarding license acquisition", Visible)),
        ...
        ("wait-for-license",
         (Toggle (\f x -> f {waitLicense=x}) (showIfTrue waitLicense),
          "wait for license to free rather than exit", Visible)),
...
showFlagsRaw flags =
        ...
        "\tgenSceMi = " ++ show (genSceMi flags) ++ ",\n" ++
        "\tgenSceMiTB = " ++ show (genSceMiTB flags) ++ ",\n" ++
        "\tgenSceMiClassic = " ++ show (genSceMiClassic flags) ++ ",\n" ++
        ...
        "\tlicenseWarning = " ++ show (licenseWarning flags) ++ ",\n" ++
        "\tlicenseType = " ++ show (licenseType flags) ++ ",\n" ++
        ...
        "\tprintExpiry = " ++ show (printExpiry flags) ++ ",\n" ++
        ...
        "\tshowLicense = " ++ show (showLicense flags) ++ ",\n" ++
        ...
        "\twaitLicense = " ++ show (waitLicense flags) ++ ",\n" ++
