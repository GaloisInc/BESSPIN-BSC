...
convertSchedules flags creation_time top_id def_clk def_rst sb_map ff_map
    ...
    let ids      = []
        ...
        create_model_def =
          define create_model_decl
                 (block $ -- record the sim state handle
                          ...
                          -- maybe do Sce-Mi setup
                          (if (genSceMi flags)
                           then [ stmt $ (var "scemi_setup") `cCall` [var "sim_hdl"] ]
                           else [])
                 )
        ...
        destroy_model_def =
          define destroy_model_decl
                 (block $ -- delete top module instance
                          (deleteInst top_blk) ++
                          -- maybe do Sce-Mi shutdown
                          (if (genSceMi flags)
                           then [ stmt $ (var "scemi_shutdown") `cCall` [var "sim_hdl"] ]
                           else [])
                 )
        ...
        parts = ListUtil.splitBy (== '.') versionnum
        year = if (null parts) then mkNULL else (mkUInt32 (read (parts!!0)))
        month = if (length parts < 2) then mkNULL else (mkUInt32 (read (parts!!1)))
        annotation = if (length parts < 3) then mkNULL else (mkStr (parts!!2))
