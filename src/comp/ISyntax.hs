...
--	getITypePositionCross,
        ...
        getInoutReset,
        getWireInfo,
        getSizeEstimate,reportSizeEstimate,
...
-- get a size estimate of the elaborated module for licenseing
getSizeEstimate :: IModule a -> (String,Int)
getSizeEstimate imod = (name,rules + local + state)
  where rules =  let (IRules _ rr) = imod_rules imod
                 in length rr
        local = length $ imod_local_defs imod
        state =  length $ imod_state_insts imod
        name =  init $ getIdString $ imod_name imod

reportSizeEstimate :: IModule a -> String -> IO()
reportSizeEstimate imod msg =
    putStrLn $ "Size Estimate, " ++ msg ++ ", " ++ show (name,rules,local,state, sum)
  where rules =  let (IRules _ rr) = imod_rules imod
                 in length rr
        local = length $ imod_local_defs imod
        state =  length $ imod_state_insts imod
        sum = rules + local + state
        name =  init $ getIdString $ imod_name imod
