...
module IExpand(id_IExpand, iExpand) where
...
import Authorization(checkoutAnyFeature)
...
handlePrim _ _ _ p e@(IAps (ICon _ (ICPrim { primOp = PrimCheckLicense })) _ [e_str]) = do
  flags <- getFlags
  errh <- getErrHandle
  (str, _) <- evalString e_str
  -- this will handle error and verbosity messages
  liftIO $ checkoutAnyFeature False errh flags str
  return (P p (icUndet itPrimUnit UNotUsed))
