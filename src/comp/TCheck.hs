...
tiExpr as td exp@(CStruct c ies) = do
    ...
    case mti of
     ...
     Nothing -> do
        ...
        tyc@(TyCon c' (Just k) ti) <- findTyCon c
        case ti of
         TIstruct ss qfs -> do
            -- find any defaults
...
tiExpr as td (CHasType (CAny {}) qt@(CQType [_] nt)) | nt == noType = do
    [PredWithPositions p poss] :=> _ <- mkQualType qt
...
tiExpl'' as0 i sc alts me oqtvts = do
    ...
    -- Trim the substitution back to the recorded point "v"
    updSubst (trimSubst v)
...
tiWithType as e = do
   v@(TVar tv) <- newTVar "tiForType" KStar e
   (ps, e') <- tiExpr as v e
   s <- getSubst
   let t = apSub s v
   -- when doTrim $ updSubst (trimSubst tv)
   t' <- expandFullType t
   return (t', ps, e')
