...
import Prelude hiding (null, lookup, length, foldr, filter)
...
instance (Show a) => Show (IntMap a) where
	...
	showsPrec _ s = showString "{" . f (toList s) . showString "}"
		where f [] = (internalError "IntMap: Unexpected []")
                      f [x] = g x
...
addMany :: [(Int, a)] -> IntMap a -> IntMap a
addMany is s = Prelude.foldr add s is
...
{-
(!) :: IntMap a -> Int -> a
t ! x = case lookup x t of Nothing -> internalError "IntMap.!: index not found"; Just y -> y
-}
...
union_C :: (a->a->a) -> IntMap a -> IntMap a -> IntMap a
union_C comb Nil t = t
union_C comb (Leaf x y) t = add_C' comb x y t
union_C comb t Nil = t
union_C comb t (Leaf x y) = add_C' comb x y t
union_C comb (Fork l r) (Fork l' r') = Fork (union_C comb l l') (union_C comb r r')
...
addMany_C :: (a->a->a) -> [(Int, a)] -> IntMap a -> IntMap a
addMany_C comb is s = Prelude.foldr (add_C comb) s is
...
trimLT :: Int -> IntMap a -> IntMap a
{-
trimLT i Nil = Nil
trimLT i t@(Leaf x y) = if x < i then t else Nil
trimLT i (Fork l r) = internalError "IntMap.trimLT: Fork"
-}
trimLT i t = fromList [ xy | xy@(x,y) <- toList t, x < i ]
