...
NOTE: THIS IS A TEMPORARY COPY OF THE DATA.MAP MODULE.
   
ONCE THE findLTE and findGTE FUNCTIONS ARE INCORPORATED INTO THE
GHC LIBS THIS COPY SHOULD GO AWAY.
   
IF YOU MAKE ANY MODIFICATIONS TO THIS FILE, YOU MUST ALSO
MAKE SURE THEY ARE ACCEPTED INTO THE GHC LIBS SO THAT WE
AREN'T STUCK WITH THIS HACK FOREVER!
...
-- | /O(log n)/. The largest key <= a given value.
findLTE :: Ord k => k -> Map k a -> Maybe (k,a)
findLTE k Tip = Nothing
findLTE k (Bin sx kx x l r)
  = case compare k kx of
      LT -> findLTE k l
      GT -> findLTE k r <|> Just (kx,x)
      EQ -> Just (kx,x)

-- | /O(log n)/. The smallest key >= a given value.
findGTE :: Ord k => k -> Map k a -> Maybe (k,a)
findGTE k Tip = Nothing
findGTE k (Bin sx kx x l r)
  = case compare k kx of
      LT -> findGTE k l <|> Just (kx,x)
      GT -> findGTE k r
      EQ -> Just (kx,x)
...
{- | /O(n)/. The expression (@'showTreeWith' showelem hang wide map@) shows
 the tree that implements the map. Elements are shown using the @showElem@ function. If @hang@ is 'True', a /hanging/ tree is shown otherwise a rotated tree is shown. If
...
{--------------------------------------------------------------------
  findLTE and findGTE
--------------------------------------------------------------------}
prop_findLTE :: Int -> Map Int Int -> Bool
prop_findLTE k m
  = let actual   = findLTE k m
        expected = case (takeWhile ((<= k).fst) (toAscList m)) of
                     [] -> Nothing
                     l  -> Just (last l)
    in actual == expected

prop_findGTE :: Int -> Map Int Int -> Bool
prop_findGTE k m
  = let actual   = findGTE k m
        expected = case (dropWhile ((< k).fst) (toAscList m)) of
                     []    -> Nothing
                     (e:_) -> Just e
    in actual == expected
-}