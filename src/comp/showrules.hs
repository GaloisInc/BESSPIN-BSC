...
import qualified MapCopy as MC  -- XXX eventually return to Data.Map
...
versionString = bluespec ++ " showrules utility, version " ++ versionnum ++
                " (" ++ build ++ versiondate ++ ")"
...
fill t0 t1 width m (code,vs) =
    ...
    in DM.adjust (MC.union (MC.fromList chgs')) code m
...
fixup :: (Integer,VCDValue,Integer,VCDValue,Integer,VCDValue,Integer) ->
         MC.Map Integer (VCDValue,ChangeReason) ->
         MC.Map Integer (VCDValue,ChangeReason)
fixup (t0,v_pre,t0',v,t1',v_post,t1) m =
    let -- determine the value at the beginning of the interval
        ...
        (val_before,at) = case c of
                            ...
                            Just (t,v) | t == t0 -> (MC.findLTE (t0-1) m, Just v)
                                       | otherwise -> (c,Nothing)
        ...
        m' = (if del_t0 then MC.delete t0 m else m) `MC.union` (MC.fromList chgs)
    in m'
...
realign t0 t1 slots rwires vmap (c,n) =
    let t = t0 + (((t1 - t0) * (fromIntegral (n+1))) `div` slots)
        ...
        move_edge m = case (MC.lookup move_from m) of
                        (Just x) -> MC.insert t x (MC.delete move_from m)
                        Nothing  -> m
