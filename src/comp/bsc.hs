...
genModule
  ...
  do
    ...
    -- check elaboration size limit
    checkBSCElaborationLimit errh
        (elaborationSizeLimit flags) (getSizeEstimate imod0)
...
simLink errh flags toplevel afilenames cfilenames = do
    ...
    start flags DFlicense
    (stat,_) <- checkoutBSCLicenses errh flags Link
    when (not stat) $ exitFail errh
    t <- dump errh flags tStart DFlicense dumpnames stat
    ...
    let name = createEncodedFullFilePath "placeholder" pwd
        prefix = (dirName name) ++ "/"
        mkEncodedName s = genFileName mkCxxName (cdir flags) prefix s
    scemi_source <- mapM (\f -> do n <- mkEncodedName f
                                   return $ getRelativeFilePath n)
                         ["scemilink"]
    let to_compile' = to_compile ++ (if (genSceMi flags) then scemi_source else [])
...
cxxLink errh flags toplevel names creation_time = do
    cxx_family <- getCxxFamily errh flags  -- probe for compiler ABI version
    ...
    let isSchedule n = (baseName n == ("model_" ++ toplevel ++ ".o"))
        ...
        compile_names =
            other_names ++
            cFiles (bluespecDir flags) cxx_family schedule_name (genSceMi flags) (genSceMiClassic flags) (genSceMiTB flags)
...
bsLibDir path cxx_family    = path ++ "/Bluesim/" ++ cxx_family ++ "/"
scemiLibDir path style cxx_family = path ++ "/SceMi/" ++ style ++ "/" ++ cxx_family ++ "/"
bluenocLibDir path cxx_family = path ++ "/BlueNoC/" ++ cxx_family ++ "/"

cFiles path cxx_family bsc_sch add_scemi add_scemiclassic add_scemitb =
    let bsc_pre_sch  = []
        bsc_post_sch = [ "lib" ++ name ++ ".a"
                       | name <- ["bskernel", "bsprim"] ]
        add_path xs  = map ((bsLibDir path cxx_family) ++) xs
        bsc_scemi    = if (add_scemiclassic) then [ "libscemiHW.a" ] else []
        bsc_scemitb  = if (add_scemitb) then [ "libscemi.a" ] else []
        bsc_style    = if (add_scemiclassic) then "Classic" else "BlueNoC"
        bsc_bluenoc  = if ((add_scemi || add_scemitb) && not add_scemiclassic) then [ "libbluenoctcp.a" ] else []
        add_scemi_path   xs = map ((scemiLibDir path bsc_style cxx_family) ++) xs
        add_bluenoc_path xs = map ((bluenocLibDir path cxx_family) ++) xs
    in (add_path bsc_pre_sch) ++
       bsc_sch ++
       (add_path bsc_post_sch) ++
       (add_scemi_path (bsc_scemi ++ bsc_scemitb)) ++
       (add_bluenoc_path bsc_bluenoc)
    ...
    (ffuncs, mod_abmis) <-
        case (mhier) of
          Left _ -> do
            ...
            -- if SceMi is indicated, include the .ba files for all VPI funcs
            -- (since we don't know which ones might be used in the Verilog)
            let scemi_bas = if (genSceMi flags)
                            then scemiABinNames flags
                            else []
                scemitb_bas = if (genSceMiTB flags)
                              then scemiTbABinNames flags
                              else []
            scemi_abis <- mapM read_abin_fn (scemi_bas ++ scemitb_bas)
            let abis = user_abis ++ scemi_abis

            -- identify which are ffunc and which are mods
            let (ffunc_abis, mod_abis) =
                    let isFF (ABinForeignFunc {}) = True
                        isFF _                    = False
                    in  partition (isFF . snd) abis
          ...
          Right (_, _, _, ffuncmap, filemap, _, mod_infos) -> do
            ...
            -- scemilink generates a Verilog fragment that uses foreign funcs
            -- which would not be visible in the design, so load them here
            let scemilink_bas = if (genSceMi flags)
                                then scemiLinkABinNames flags
                                else []
            let scemilinktb_bas = if (genSceMiTB flags)
                                  then scemiTbABinNames flags
                                  else []
            scemi_abis   <- mapM read_abin_fn scemilink_bas
            scemitb_abis <- mapM read_abin_fn scemilinktb_bas
            let scemi_ffuncs =
                    [ (abffi_foreign_func abfi)
                       | (_, ABinForeignFunc abfi _) <- scemi_abis ]
            let scemitb_ffuncs =
                    [ (abffi_foreign_func abfi)
                       | (_, ABinForeignFunc abfi _) <- scemitb_abis ]
            -- Union, in case there is overlap
            -- XXX check for same name but different func?
            let ffuncs = scemi_ffuncs `union` scemitb_ffuncs `union` user_ffuncs
...
vSimLink errh flags toplevel prefix vfiles ofiles = do
    ...
    cxx_family <- getCxxFamily errh flags  -- probe for compiler ABI version
    scemi_vlog <- genFileName mkNameWithoutSuffix (vdir flags) prefix "scemilink.vlog_fragment"
    let bsdir = bluespecDir flags
        libdirflags = map ("-L "++) (cLibPath flags)
        userlibs = map ("-l "++) (cLibs flags)
        scemiflags = if (genSceMi flags)
                     then if (genSceMiClassic flags)
                          then let lp = scemiLibDir bsdir bsc_style cxx_family
                               in [ "-L " ++ lp ++ " -l scemiHW -D BSV_SCEMI_LINK=\\\"" ++ scemi_vlog ++ "\\\"" ]
                          else let lp = bluenocLibDir bsdir cxx_family
                               in [ "-L " ++ lp ++ " -l bluenoctcp -D BSV_SCEMI_LINK=\\\"" ++ scemi_vlog ++ "\\\"" ]
                     else []
        bsc_style    = if (genSceMiClassic flags) then "Classic" else "BlueNoC"
        scemitbflag1 = if (genSceMiTB flags)
                       then let lp = scemiLibDir bsdir bsc_style cxx_family
                            in [ "-L " ++ lp ++ " -l scemi" ]
                       else []
        scemitbflags = if ((genSceMiTB flags) && not (genSceMiClassic flags))
                       then let lp = bluenocLibDir bsdir cxx_family
                            in scemitbflag1 ++ [ " -L " ++ lp ++ " -l bluenoctcp" ]
                       else scemitbflag1
...
getVerilogSim errh flags = do
    ...
    let vsim_path | any (== '/') vsim_name = vsim_name
                  | otherwise = (bluespecDir flags ++ "/bin/bsc_build_vsim_" ++
                                 vsim_name)
...
findAllAvailableSims flags =
    do let script_dir = bluespecDir flags ++ "/bin"
...
vGenFFuncs errh flags t prefix cfilenames_unique ffuncs = do
      ...
      -- if necessary, copy vpi wrappers for SceMi
      -- XXX This should be replaced with general handling for VPI wrappers
      -- XXX generated with previous compile commands (such as in BSVSource)
      when (genSceMi flags) $
      ...
      -- if necessary, copy vpi wrappers for SceMiTB
      -- XXX This should be replaced with general handling for VPI wrappers
      -- XXX generated with previous compile commands (such as in BSVSource)
      when (genSceMiTB flags) $
           do let to = fromMaybe "." (vdir flags)
              mapM_ (\src -> do let dst = to ++ "/" ++ (baseName src)
                                copyFileCatch errh src dst)
