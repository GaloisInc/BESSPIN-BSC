...
import Authorization(WSLicMode(..), checkoutWSLicenses,
                     BSCLicMode(..), checkoutBSCLicenses, checkinLicense)
...
licTest :: Flags -> WSLicMode -> IO ()
licTest flgs wsmode = do
  errs <- checkoutWSLicenses globalErrHandle flgs wsmode
  if null errs then return () else eMsgsToTcl errs
...
versionNum [] = return $ [versionnum, versiondate, buildVersion]
...
debugGrammar = (tclcmd "debug" namespace helpStr "") .+.
    ...
    where helpStr = "Internal debugging command"
          ...
          coGrammar      = kw "co"      "co" ""
          ciGrammar      = kw "ci"      "ci" ""
                           .+. (atLeast 1 (arg "xxx" StringArg "lname"))
...
tclDebug ["imports"] = do
  ...
  return "Done"
----------
-- license checks (for debug)
tclDebug ["co"] = do
  flags <- getGFlags
  (stat,_) <- checkoutBSCLicenses globalErrHandle flags Compile
  return (show stat)
----------
tclDebug ("ci":args) = do
  flags <- getGFlags
  let verb = showLicense flags
  mapM_ (checkinLicense verb) args
  return "Done"
----------
